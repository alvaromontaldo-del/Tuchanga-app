-- =============================================================================
-- Soft-delete de chat → conversación NUEVA al recontactar
--
-- Problema: "eliminar" solo ocultaba para un usuario (conversation_hides) y
-- find_or_create_conversation reutilizaba el mismo hilo (UNIQUE cliente/trabajador),
-- así que al contactar de nuevo se veía todo el histórico.
--
-- Solución:
--   1) conversations.deleted_at
--   2) UNIQUE parcial: solo 1 hilo ACTIVO por par
--   3) hide_conversation marca deleted_at + oculta para AMBOS
--   4) find_or_create ignora soft-deleted y crea fila nueva
--   5) Unread / listados excluyen deleted_at
-- =============================================================================
...
COMMENT ON COLUMN public.conversations.deleted_at IS
  'Soft-delete: si no es NULL el hilo no se muestra ni se reutiliza; un nuevo contacto crea otra conversación.';

-- Quitar UNIQUE duro (permite historial soft-deleted + hilo activo nuevo)
ALTER TABLE public.conversations
...
-- ---------------------------------------------------------------------------
-- Soft-delete para ambos participantes (reusa conversation_hides + deleted_at)
-- ---------------------------------------------------------------------------
...
-- RPC usada por la app al "eliminar" chat
...
-- ---------------------------------------------------------------------------
-- Abrir/crear: solo hilos activos; si el anterior está soft-deleted → uno nuevo
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Solo reutilizar hilo activo (no soft-deleted y no oculto por nadie).
  ...
  -- Hilos viejos ocultos / inconsistentes: cerrarlos para no reaparecer.
...
-- ---------------------------------------------------------------------------
-- Unread: no contar hilos soft-deleted
-- ---------------------------------------------------------------------------
