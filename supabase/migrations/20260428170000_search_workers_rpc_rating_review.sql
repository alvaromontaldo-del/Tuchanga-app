-- Extiende la RPC de búsqueda para incluir rating_average y review_count.
-- Requiere columnas en profiles (ver migración profile_rating_from_reviews).

-- Postgres no permite cambiar el return type (OUT params) con CREATE OR REPLACE.
-- Para agregar columnas al RETURNS TABLE, hay que dropear y recrear la función.
drop function if exists public.search_workers_for_client(
  double precision,
  double precision,
  text,
  text[],
