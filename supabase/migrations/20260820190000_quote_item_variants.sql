-- Cotización: hasta 3 variantes por ítem del pedido (marcas/modelos).
...
BEGIN
  ...
  WHERE conrelid = 'public.quote_items'::regclass
    AND contype = 'u'
    AND pg_get_constraintdef(oid) ILIKE '%quote_id%request_item_id%'
    AND pg_get_constraintdef(oid) NOT ILIKE '%variant_index%'
  LIMIT 1;
  IF cname IS NOT NULL THEN
    EXECUTE format('ALTER TABLE public.quote_items DROP CONSTRAINT %I', cname);
  END IF;
END $$;
...
COMMENT ON COLUMN public.quote_items.variant_index IS
  '1..3 opciones del comercio para el mismo request_item (marcas/precios).';
COMMENT ON COLUMN public.quote_items.variant_label IS
  'Etiqueta opcional de la variante (ej. marca A).';

-- create_material_checkout: aceptar quote_item_ids (variantes) además de item_ids (request).
